...
last_verified: 2026-09-16 10:20:00
verified_commit: working-tree
---

# FEAT-002：主工具栏 Everything 搜索与结果标签页

## 1. 文档说明

### 1.1 目的与范围

定义 Explorer++ 与本机 Everything 的搜索集成：主工具栏固定显示搜索入口，结果异步呈现在独立结果标签页中。本文覆盖工具栏输入、Everything IPC、结果模型、结果标签页、打开与右键行为、持久化、隐私、错误处理和验收标准，不包含 Everything 本体安装或数据库管理。
...
- 提交查询时创建或复用一个 Everything 结果标签页，不再创建右侧辅助窗格。
- 结果遵循 Everything 的标准鼠标语义：单击选择，双击或 Enter 直接打开，右键显示系统 Shell 上下文菜单。
- 结果列表默认启用浅色隔行底色，可在搜索按钮下拉菜单中开关并持久化。
...
| 2026-09-16 | 结果从右侧窗格迁移为标签页；增加直接打开、Shell 右键菜单和可配置隔行底色 |
...
- 菜单关闭后，按钮工具提示显示当前范围和已启用选项；结果标签标题显示查询状态和结果数量。
...
### 2.3 Everything 结果标签页

- 第一次提交查询时在当前活动面板创建并选中一个名为 `Everything - <查询>` 的结果标签页。
- 同一窗口的后续查询复用该结果标签；用户关闭后，下次查询重新创建。
- 结果标签使用普通标签的关闭、切换和排序机制，不占用窗口右侧宽度。
- 标签内部仍使用轻量的 `LVS_OWNERDATA` 虚拟列表；承载标签的普通 Shell 列表在结果标签选中时隐藏。
...
- “隔行底色”默认开启：未选中的奇数行使用主题适配的轻微底色；选中行始终保留系统选中样式。
...
| 双击或 Enter：文件 | 使用系统默认程序直接打开文件 |
| 右键 | 显示该文件或文件夹的 Shell 上下文菜单；其中“打开”遵循上述直接打开语义 |
...
| `EverythingSearchTab` | 结果标签身份、虚拟列表显隐和焦点切换 |
| `ConfigRegistryStorage` / `ConfigXmlStorage` | 搜索选项及隔行底色偏好持久化 |
...
```text
Closed ──提交查询──> SearchingTab ──成功──> ShowingResults
  │                       │  └─失败──> ErrorWithPreviousResults
  └─重新查询时创建标签────┘

ShowingResults ──新查询──> SearchingTab ──成功──> ShowingResults(replaced)
ShowingResults/Error ──关闭标签──> Closed
```
...
- Everything 结果作为普通标签存在于提交查询时的活动面板，不额外分割主窗口工作区。
- 选中结果标签时，虚拟结果列表覆盖该承载标签的普通 Shell 列表；切换到其他标签时立即隐藏。
- 双面板下，结果标签只属于创建它的面板；关闭或切换双面板时沿用现有标签迁移规则。
- 文件夹树、Display Window 和双面板宽度不再受 Everything 结果影响。

### 3.9 存储

搜索范围、五个匹配选项及 `everythingAlternateRowColors` 作为应用级用户偏好持久化，新窗口继承它们；已经提交的请求保留自己的查询设置快照。XML 和注册表读取必须允许字段缺失；缺失时采用“当前文件夹范围、仅忽略变音标记和隔行底色开启”。结果标签和结果数据不跨进程恢复。旧版窗格可见性及宽度字段仅为读取兼容保留，保存时写入隐藏状态。
...
| 情况 | 用户可见行为 | 数据行为 |
|---|---|---|
| Everything 未运行 | 结果标签标题显示 Everything 未运行 | 保留旧结果 |
...
| `MainWndSwitch.cpp` | 虚拟列表通知、双击/Enter、右键菜单和隔行自绘 |
| `EverythingSearch*` 文件 | 设置、查询构造器、控制器、IPC 和结果模型 |
| `MsgHandler.cpp` | 结果标签创建/复用、列表覆盖布局、直接打开和 Shell 菜单 |
| XML/Registry 配置存储 | 搜索选项和隔行底色开关 |
| `TabHandler.cpp` | 结果标签焦点/关闭清理及最后标签替换 |
...
- 双击文件或按 Enter 会使用系统默认程序直接打开文件。
- 双击文件夹会在活动面板新标签中打开文件夹。
- 右键会显示目标的系统 Shell 上下文菜单，菜单“打开”与双击语义一致。
- 最后一个结果标签或普通标签被关闭时，窗口保持运行并创建默认标签。
- Everything 不存在、未运行、超时和协议异常均有明确且可恢复的状态。
- 隔行底色开关跨重启恢复；选中行不被自绘底色覆盖。
- 与垂直 Display Window 和双面板同时开启时，结果标签不额外占用窗口宽度。
...
- 预览、内容搜索模板、查询历史和多个并存结果标签不在首版范围内。
