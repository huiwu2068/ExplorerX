...
[vcpkg](https://vcpkg.io/) is used to manage dependencies. Before you can build ExplorerX, initialize vcpkg:

- Clone this repo with submodules enabled:
```
git clone --recurse-submodules https://github.com/huiwu2068/explorerplusplus.git
cd explorerplusplus
```
...
msbuild Explorer++\ExplorerX\ExplorerX.vcxproj /m /p:Configuration=Debug /p:Platform=x64 /p:SolutionDir=%CD%\Explorer++\
```
...
& cmd.exe /d /c 'call "C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\Common7\Tools\VsDevCmd.bat" -arch=x64 -host_arch=x64 && msbuild ".\Explorer++\ExplorerX\ExplorerX.vcxproj" /m /p:Configuration=Debug /p:Platform=x64 /p:SolutionDir=%CD%\Explorer++\ /p:UseMultiToolTask=true /p:CL_MPCount=%NUMBER_OF_PROCESSORS%'

# Build the 64-bit Release configuration.
& cmd.exe /d /c 'call "C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\Common7\Tools\VsDevCmd.bat" -arch=x64 -host_arch=x64 && msbuild ".\Explorer++\ExplorerX\ExplorerX.vcxproj" /m /p:Configuration=Release /p:Platform=x64 /p:SolutionDir=%CD%\Explorer++\ /p:UseMultiToolTask=true /p:EnforceProcessCountAcrossBuilds=true /p:CL_MPCount=%NUMBER_OF_PROCESSORS% /p:LinkAdditionalOptions="/CGTHREADS:%NUMBER_OF_PROCESSORS%"'
...
```powershell
& .\Explorer++\ExplorerX\x64\Debug\ExplorerX.exe
```
...
The Debug x64 executable is written to `Explorer++\ExplorerX\x64\Debug\ExplorerX.exe`; the Release x64 executable is written to `Explorer++\ExplorerX\x64\Release\ExplorerX.exe`.
...
Building the program in release mode will also build all of the translations. The resulting DLLs can then be used with ExplorerX.
