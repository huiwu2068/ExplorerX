...
std::unique_ptr<ResizableDialogHelper> FilesFoldersOptionsPage::InitializeResizeDialogHelper()
{
	...
	controls.emplace_back(GetDlgItem(GetDialog(), IDC_SETTINGS_CHECK_LAZY_TABS), MovingType::None,
		SizingType::Horizontal);
	controls.emplace_back(GetDlgItem(GetDialog(), IDC_SETTINGS_CHECK_EXPANDABLE_FOLDERS), MovingType::None,
		SizingType::Horizontal);
...
void FilesFoldersOptionsPage::SaveSettings()
{
	...
	m_config->enableExpandableFolders =
		(IsDlgButtonChecked(GetDialog(), IDC_SETTINGS_CHECK_EXPANDABLE_FOLDERS) == BST_CHECKED);
