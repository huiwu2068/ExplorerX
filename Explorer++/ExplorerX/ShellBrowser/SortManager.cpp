...
std::vector<int> ShellBrowserImpl::GetAncestorPath(int internalIndex) const
{
	std::vector<int> path;
...
{
	if (InternalIndex1 == InternalIndex2)
	{
		return 0;
	}

	auto it1 = m_itemInfoMap.find(InternalIndex1);
	auto it2 = m_itemInfoMap.find(InternalIndex2);
	if (it1 == m_itemInfoMap.end() || it2 == m_itemInfoMap.end())
	{
		return 0;
	}

	// If neither item is an inline expanded child, directly compare them
	if (!it1->second.isChildItem && !it2->second.isChildItem)
	{
		return SortItemDirect(InternalIndex1, InternalIndex2);
	}

	std::vector<int> path1 = GetAncestorPath(InternalIndex1);
	std::vector<int> path2 = GetAncestorPath(InternalIndex2);
	...
	for (size_t i = 0; i < minLen; ++i)
	{
		if (path1[i] != path2[i])
		{
			// Diverged at this level: compare the two siblings using the current sort criteria
			return SortItemDirect(path1[i], path2[i]);
		}
	}

	// One is an ancestor of the other: parent always precedes child!
...
int ShellBrowserImpl::SortItemDirect(int InternalIndex1, int InternalIndex2) const
{
