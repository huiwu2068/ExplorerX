...
void ShellBrowserImpl::OnListViewGetDisplayInfo(LPARAM lParam)
{
	...
	if (m_folderSettings.viewMode == +ViewMode::Details)
	{
		...
		{
			plvItem->mask |= LVIF_INDENT;
			plvItem->iIndent = IsExpandableFoldersEnabled() ? (it->second.depth + 1) : 0;
		}
	}
...
void ShellBrowserImpl::OnListViewKeyDown(const NMLVKEYDOWN *lvKeyDown)
{
	...
	case VK_RIGHT:
		if (m_folderSettings.viewMode == +ViewMode::Details && IsExpandableFoldersEnabled())
		{
			...
			if (selected != -1)
			{
				const auto &item = GetItemByIndex(selected);
				if ((item.wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && !item.isExpanded)
				{
					ExpandFolder(selected);
				}
			}
		}
		break;
	...
	case VK_LEFT:
		if (m_folderSettings.viewMode == +ViewMode::Details && IsExpandableFoldersEnabled())
		{
			...
			if (selected != -1)
			{
				...
				if ((item.wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && item.isExpanded)
				{
					CollapseFolder(selected);
				}
				else if (item.isChildItem && item.parentInternalIndex != -1)
				{
					auto parentVisualIdx = LocateItemByInternalIndex(item.parentInternalIndex);
					if (parentVisualIdx)
					{
						ListViewHelper::SelectItem(m_listView, *parentVisualIdx, true);
					}
				}
			}
		}
...
{
	DWORD drawStage = listViewCustomDraw->nmcd.dwDrawStage;
	...
	if ((drawStage & CDDS_ITEM) && (drawStage & CDDS_PREPAINT))
	{
	...
	if ((drawStage & CDDS_ITEM) && (drawStage & CDDS_POSTPAINT))
	{
		if (m_folderSettings.viewMode == +ViewMode::Details && IsExpandableFoldersEnabled())
		{
			...
			if (itemIndex >= 0 && itemIndex < ListView_GetItemCount(m_listView))
			{
				...
				if (itemInfo.wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
				{
					...
					if (ListView_GetItemRect(m_listView, itemIndex, &rcIcon, LVIR_ICON))
					{
						...
						rcChevron.top = rcIcon.top + (rcIcon.bottom - rcIcon.top - glyphSize) / 2;
						rcChevron.bottom = rcChevron.top + glyphSize;

						DrawChevron(listViewCustomDraw->nmcd.hdc, rcChevron, itemInfo.isExpanded);
					}
				}
			}
		}
