...
bool ShellBrowserImpl::IsExpandableFoldersEnabled() const
{
	return m_config ? m_config->enableExpandableFolders : true;
}

void ShellBrowserImpl::ToggleFolderExpanded(int itemIndex)
{
	...
	{
		CollapseFolder(itemIndex);
	}
	else
	{
		ExpandFolder(itemIndex);
	}
}

void ShellBrowserImpl::ExpandFolder(int itemIndex)
{
	if (itemIndex < 0 || itemIndex >= ListView_GetItemCount(m_listView))
	{
		return;
	...
	int parentInternalIndex = GetItemInternalIndex(itemIndex);
	auto parentIt = m_itemInfoMap.find(parentInternalIndex);
	if (parentIt == m_itemInfoMap.end())
	{
		return;
	}

	ItemInfo_t &parentInfo = parentIt->second;
	if (parentInfo.isExpanded)
	{
		return;
	}

	if (!(parentInfo.wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
	{
		return;
	}
	...
	for (auto &subItem : subItems)
	{
		...
		subItem.parentInternalIndex = parentInternalIndex;
		subItem.isChildItem = true;
		...
		BasicItemInfo_t basicItemInfo = getBasicItemInfo(newId);
		std::wstring filename = ProcessItemFileName(basicItemInfo, m_config->globalFolderSettings);

		LVITEM lv{};
		lv.mask = LVIF_TEXT | LVIF_IMAGE | LVIF_PARAM | LVIF_INDENT;
		lv.iItem = insertVisualPos++;
		lv.iIndent = IsExpandableFoldersEnabled() ? (subItem.depth + 1) : 0;
		lv.pszText = filename.data();
		lv.iImage = I_IMAGECALLBACK;
		lv.lParam = newId;

		ListView_InsertItem(m_listView, &lv);
...
void ShellBrowserImpl::CollapseFolder(int itemIndex)
{
	if (itemIndex < 0 || itemIndex >= ListView_GetItemCount(m_listView))
	{
		return;
	...
	{
		int internalIndex = GetItemInternalIndex(i);
		if (IsDescendantOf(internalIndex, parentInternalIndex))
		{
			ListView_DeleteItem(m_listView, i);
			internalIndicesToRemove.push_back(internalIndex);
			m_directoryState.numItems--;
		}
	}

	for (int internalIndex : internalIndicesToRemove)
	{
		m_itemInfoMap.erase(internalIndex);
	}

	InvalidateRect(m_listView, nullptr, FALSE);
}

bool ShellBrowserImpl::IsDescendantOf(int internalIndex, int targetParentInternalIndex) const
{
	int current = internalIndex;
	while (current != -1)
	{
		auto it = m_itemInfoMap.find(current);
		if (it != m_itemInfoMap.end() && it->second.isChildItem && it->second.parentInternalIndex != -1)
		{
			if (it->second.parentInternalIndex == targetParentInternalIndex)
			{
				return true;
			}
			current = it->second.parentInternalIndex;
		}
		else
		{
			break;
...
void ShellBrowserImpl::DrawChevron(HDC hdc, const RECT &rcChevron, bool isExpanded)
{
	...
	HTHEME hTheme = OpenThemeData(m_listView, L"Explorer::TreeView");
	if (!hTheme)
	{
		hTheme = OpenThemeData(m_listView, L"TreeView");
	}

	if (hTheme)
	{
		// TVP_GLYPH = 2, GLPS_CLOSED = 1, GLPS_OPEN = 2
		int state = isExpanded ? 2 : 1;
		HRESULT hr = DrawThemeBackground(hTheme, hdc, 2, state, &rcChevron, nullptr);
		if (SUCCEEDED(hr))
		{
			themeDrawn = true;
		}
	...
	if (!themeDrawn)
	{
		int cx = (rcChevron.left + rcChevron.right) / 2;
		int cy = (rcChevron.top + rcChevron.bottom) / 2;
		HPEN hPen = CreatePen(PS_SOLID, 2, RGB(90, 90, 90));
		...
		{
			MoveToEx(hdc, cx - 2, cy - 4, nullptr);
			LineTo(hdc, cx + 3, cy);
